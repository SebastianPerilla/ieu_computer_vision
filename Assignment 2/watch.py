...
def draw_polygon(img, center, radius, color=EMERALD, thickness=2):
    points = [
        (
...
def draw_bolts(img, center, radius):
    for angle in [(i * (2 * np.pi / 8)) + (np.pi / 8) for i in range(8)]:
        ...
        cv.circle(img, (x, y), 7, EMERALD, -1, lineType=cv.LINE_AA)
...
def draw_logo(img, center):
    ...
    cv.putText(img, text, pos, font, scale, EMERALD, thickness, cv.LINE_AA)
...
def draw_hour_markers(img, center, radius):
    for i in range(12):
        ...
        cv.line(img, start, end, EMERALD, 3, cv.LINE_AA)


def draw_clock_hands(img, center, radius):
    now = datetime.now()
    hands = [
        ((now.hour % 12 + now.minute / 60) * (2 * np.pi / 12) - (np.pi / 2), 0.5, 4, EMERALD),
        ((now.minute + now.second / 60) * (2 * np.pi / 60) - (np.pi / 2), 0.7, 3, EMERALD),
    ...
    cv.circle(img, center, 8, EMERALD, -1, lineType=cv.LINE_AA)
...
def draw_watch_links(img, center, radius, top=True):
    ...
    cv.line(img, (x1, y1), (x1 + 60, end_y), EMERALD, 2, cv.LINE_AA)
    cv.line(img, (x2, y2), (x2 - 60, end_y), EMERALD, 2, cv.LINE_AA)
    cv.line(img, (x1 + 60, end_y), (x2 - 60, end_y), EMERALD, 2, cv.LINE_AA)
    
    link_y = end_y + 30 if not top else end_y - 30
    cv.line(img, (x1 + 60, end_y), (x1 + 65, link_y), EMERALD, 2, cv.LINE_AA)
    cv.line(img, (x2 - 60, end_y), (x2 - 65, link_y), EMERALD, 2, cv.LINE_AA)
    cv.line(img, (x1 + 65, link_y), (x2 - 65, link_y), EMERALD, 2, cv.LINE_AA)
    ...
    while 10 < current_y + link_height < HEIGHT - 10 if not top else current_y - link_height > 10:
        cv.line(img, (left_x, current_y), (left_x, current_y + link_height if not top else current_y - link_height), EMERALD, 2, cv.LINE_AA)
        cv.line(img, (right_x, current_y), (right_x, current_y + link_height if not top else current_y - link_height), EMERALD, 2, cv.LINE_AA)
        cv.line(img, (left_x, current_y + link_height if not top else current_y - link_height), (right_x, current_y + link_height if not top else current_y - link_height), EMERALD, 2, cv.LINE_AA)
